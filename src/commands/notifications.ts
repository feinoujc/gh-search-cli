...
function filteredBaseOptions(): Pick<typeof BaseCommand['flags'], 'api-base-url' | 'api-token' | 'json' | 'open' > {
...
export default class Notifications extends Command {
  ...
  static flags = {
    ...
    repo: flags.string({
      description: 'Filter notifications to a repository, required with --owner flag'
    }),
  }
  ...
  async run() {
    ...
    const print = (resp: ApiResponse, opts: { json?: boolean, open?: boolean }) => {
      if (opts.json) {
        ...
        if (opts.open) {
          const firstLink = resp.items.map(({subject}) => subject.latest_comment_html_url).find(Boolean)
          if (firstLink) {
