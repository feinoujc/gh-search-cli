...
async function fetchAndAssignSubjectHtmlUrl(
	subject: any,
	opts: ApiOptions,
): Promise<void> {
	try {
...
async function parseResponse(
	...
	await Promise.all(
		subjects.map((subject: any) => fetchAndAssignSubjectHtmlUrl(subject, opts)),
	);
...
export default async function getNotifications(
	...
	const url =
		owner && repo ? `/repos/${owner}/${repo}/notifications` : '/notifications';
	const options = {
		auth: {
			user: '',
			password: opts.apiToken,
